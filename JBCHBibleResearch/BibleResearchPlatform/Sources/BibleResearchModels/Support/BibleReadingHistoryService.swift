...
public enum BibleReadingHistoryService {
    ...
    /// 책/장(선택적으로 절까지)을 하나 기록한다. 호출부(`BibleReadingViewModel.selectBook`/
    /// `goToChapter`/`navigate(toHistory:)`/`recordVerseHistory`)가 **사용자가 실제로
    /// 이동을 선택했을 때만** 불러야 한다 — `BibleReadingViewModel.init`이
    /// `LastBiblePositionTracker`로 마지막 위치를 "복원"하는 경우까지 기록하면, 다른
    /// 화면과 성경 조회를 오갈 때마다 같은 위치가 계속 이력에 쌓여 "조회 이력"이라는
    /// 취지와 어긋난다.
    ///
    /// 바로 직전 항목과 book/chapter/verse가 완전히 같으면 새로 기록하지 않는다 —
    /// 화면 재진입 등으로 같은 좌표에 대해 `selectBook`이 반복 호출되는 경우
    /// 이력이 의미 없이 중복되는 것을 막기 위한 최소한의 방어다. [2026-08-26 수정]
    /// verse도 비교 대상에 넣었다 — 그렇지 않으면 "3장으로 이동(verse=nil)" 직후
    /// "3장 5절 확대보기(verse=5)"처럼 서로 다른 이벤트인데도 book/chapter만 같다는
    /// 이유로 뒤엣것이 통째로 무시돼 버린다.
